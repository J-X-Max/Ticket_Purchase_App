<template>
    <div class="container">
        ...
        <div class="button-box"><button @click="handleLogin" :class="{ 'active': islogin }">登录</button></div>
        <span class="tips">收不到验证码</span>
    </div>
    <div class="agreement-box">
        <input v-model="isagreement" type="checkbox" name="User_Agreement">
        <div class="text">
            已阅读并同意<a href="#/user/privacy-policy">《卖座电影隐私政策》</a>和
            <a href="#/user/you-should-know">《卖座电影用户协议》</a>,并于清晰理解上述文件中免除和限制责任、法律适用和争议解决等粗体或下划线条款
        </div>
    </div>
</template>
...
const handleLogin = async () => {
    ...
    try {
        const res1 = await http.post('/gateway/', { extra: {}, imgCode: "", imgKey: "", mobile: telnumber.value, smsCode: smscode.value },
            {
                headers: {
...
watch([telnumber, graphicCode, smscode], ([newValtel, newgraphicCode, newsmscode]) => {
